...
(function (global, factory) {
  ...
  /* Internal Use Only */
    ...
    if (_typeof2(name) === "object" || typeof name === "function") {
      if (pvt) {
        ...
        if (needAjaxValidation && ($.isEmptyObject(messages) || data.submitting)) {
          ...
          $.ajax({
            ...
            success: function success(msgs) {
              if (msgs !== null && _typeof2(msgs) === 'object') {
                $.each(data.attributes, function () {
...
;
var questions = document.body.querySelectorAll('.single-question'); // ************** utm **************

var utmInputs = document.body.querySelectorAll('.utm');
...
var getUtmParameter = function getUtmParameter(sParam) {
  ...
  for (i = 0; i < urlVariables.length; i++) {
    utm = urlVariables[i].split('=');

    if (utm[0] === sParam) {
      return utm[1] === undefined ? true : decodeURIComponent(utm[1]);
    }
  }
};

var tech = getUtmParameter('utm_campaign');
setTimeout(function () {
  if (tech) {
    utmInputs.forEach(function (item) {
      return item.value = tech;
    });
  } else {
    utmInputs.forEach(function (item) {
      return item.value = 'direct';
    });
  }
}, 600); // ************** запрет повторной отправки **************
...
document.body.querySelector('form').addEventListener('keydown', function (evt) {
  if (evt.key === 13) {
    evt.preventDefault();
  }
}); // ************** переключение на вопросы **************

var bannerButton = document.body.querySelector('.banner_button');
var disablePage = document.body.querySelector('.disable_page');
var banner = document.body.querySelector('.banner');
bannerButton.addEventListener('click', function (evt) {
  evt.preventDefault();
  var cookie = getCookie('credit-cards.by');

  if (typeof cookie != "undefined") {
    disablePage.classList.add('active');
  } else {
...
var prevButtons = document.body.querySelectorAll('.prev');
...
nextButtons.forEach(function (button) {
  button.addEventListener('click', function (evt) {
    evt.preventDefault();
    activeQuestion++;
    changeActiveClassOnSingleQuestion();
  });
});
...
var changeActiveClassOnSingleQuestion = function changeActiveClassOnSingleQuestion() {
  questions.forEach(function (question) {
    return question.classList.remove('active');
  });
  questions[activeQuestion].classList.add('active');
}; // ************** добавление шагов **************


var questionSteps = document.body.querySelectorAll('.question_steps');
questionSteps.forEach(function (item, i) {
  var number = item.querySelector('.question_number');
  var count = item.querySelector('.question_count');
  number.textContent = i + 1;
  count.textContent = questions.length;
}); // ************** валидация все ли поля имеют класс valid **************

var changeFlagHandler = function changeFlagHandler(inputs, submitButton) {
...
questions.forEach(function (question) {
  ...
  var isDisableButton = question.querySelector('.isDisableButton');
  ...
  question.addEventListener('click', function () {
    return changeFlagHandler(inputs, isDisableButton);
  });
...
}); // ************** добавление класса valid **************

var isValidInput = document.body.querySelectorAll('.isValidInput');
var accepting = document.body.querySelector('#accepting');
isValidInput.forEach(function (item) {
...
accepting.addEventListener('change', function () {
  if (accepting.checked) {
    accepting.classList.add('valid');
  } else {
    accepting.classList.remove('valid');
  }
}); // ************** маски **************

var phoneInputs = document.body.querySelectorAll('.telmask');
$(function () {
  $(".telmask").inputmask({
...
phoneInputs.forEach(function (input) {
  var validErrorMessage = input.parentElement.querySelector('.valid-error');
  input.addEventListener('keyup', function () {
    var tel = input.value;
    var codes = ['25', '29', '33', '44'];

    if (parseInt(input.value[input.value.length - 1])) {
      var code = tel.replace(/[^+\d]/g, '').substring(4, 6);
      var isTrueCode = codes.some(function (item) {
        return item === code;
      });
      ...
      if (isTrueCode) {
        ...
        validErrorMessage.textContent = '';
      } else {
        input.classList.remove('valid');
        input.classList.add('invalid');
        validErrorMessage.textContent = '* Пожалуйста, введите корректный код мобильного оператора';
      }
    } else {
      input.classList.remove('valid');
      input.classList.remove('invalid');
...
$(function () {
  document.addEventListener('wpcf7mailsent', function (event) {
    setCookie('credit-cards.by', 'send', {});
  }, false);
});